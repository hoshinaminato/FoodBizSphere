    ...
    loadProjects().then(loadedProjects => {
      const migrated = loadedProjects.map(p => ({
        ...
        evaluations: (p.evaluations || []).map(e => ({
          ...e,
          area: e.area ?? 0,
          outdoorArea: e.outdoorArea ?? 0,
          fumeType: e.fumeType ?? 'none',
          rent: e.rent ?? 0,
          paymentMethod: e.paymentMethod ?? 1,
          deposit: e.deposit ?? 0,
          transferFee: e.transferFee ?? 0,
          franchiseFee: e.franchiseFee ?? 0,
          renovationFee: e.renovationFee ?? 0,
          equipmentFee: e.equipmentFee ?? 0,
          initialMaterialFee: e.initialMaterialFee ?? 0,
          monthlyLabor: e.monthlyLabor ?? 0,
          monthlyUtilities: e.monthlyUtilities ?? 0,
          dineInGrossMargin: e.dineInGrossMargin ?? e.grossMargin ?? 0,
          takeoutGrossMargin: e.takeoutGrossMargin ?? e.grossMargin ?? 0,
          dineInEstimatedDailyRevenue: e.dineInEstimatedDailyRevenue ?? e.estimatedDailyRevenue ?? 0,
          takeoutEstimatedDailyRevenue: e.takeoutEstimatedDailyRevenue ?? 0,
          averageTransactionValue: e.averageTransactionValue ?? 0,
          location: e.location || { country: 'china', province: '', city: '', address: '' },
          images: e.images || { dish: [], menu: [], interior: [], exterior: [], reviews: [] }
        })),
  ...
  const createEvaluation = (projectId: string) => {
    setProjects(prev => {
      ...
      const newEval: Evaluation = {
        ...
        monthlyUtilities: 0,
        dineInGrossMargin: 0,
        takeoutGrossMargin: 0,
        dineInEstimatedDailyRevenue: 0,
        takeoutEstimatedDailyRevenue: 0,
        averageTransactionValue: 0,
